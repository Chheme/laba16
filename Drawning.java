import java.awt.*;
import javax.swing.JFrame;

public class Drawning extends Canvas{
    public static void main(String[] args){
        JFrame frame = new JFrame("My Drawning");
        Canvas canvas  = new Drawning();
        canvas.setSize(600,400);
        frame.add(canvas);
        frame.pack();
        frame.setVisible(true);
    }

    @Override
    public void paint(Graphics g){
        g.drawLine(25, 300, 425, 300);
        g.drawRect(50,150,150,150);
        g.drawRect(75,200,50,50);
        g.drawLine(25, 175, 125, 75);
        g.drawLine(125, 75, 225, 175);
        g.drawLine(150, 100, 150, 75);
        g.drawLine(150, 75, 175, 75);
        g.drawLine(175, 75, 175, 125);
        g.drawArc(275,50,150,200,280,341);
        g.drawLine(339, 300, 339, 250);
        g.drawLine(361, 300, 361, 250);
}


}
